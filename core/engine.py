# core/engine.py — Enterprise WhatsApp Clinic Engine (V4.6.0)
# Fixes:
# ✅ Patient details no longer drop to main menu
# ✅ Slot ranges corrected (8AM–3PM, 5PM–10PM, 11PM–2AM)
# ✅ "99 Speak to Reception" consistent everywhere
# ✅ Privacy notice shown ONCE only (first outbound message per session), including specialty inquiry replies
# ✅ Booking summary "2 Modify" goes to DATE (not specialty list)
#
# SAFE PHASE 10 INTEGRATION:
# ✅ Knowledge Base answers only for informational questions
# ✅ Does NOT override booking / reschedule / cancel transaction states
# ✅ Safe fallback to normal menu if KB has no good answer
...
ENGINE_MARKER = "CLINIC_ENGINE_V4_6_0"
...
try:
    from core.knowledge_retriever import build_knowledge_answer  # type: ignore
except Exception:
    build_knowledge_answer = None
...
def _maybe_privacy_once(sess: Dict[str, Any], lang: str) -> str:
    if bool(sess.get("privacy_notice_sent")):
        return ""
    sess["privacy_notice_sent"] = True
...
def _confirmation(sess: Dict[str, Any], lang: str) -> str:
    ...
    if lang == "ar":
        return (
            ...
            + expiry_line
            + "\n\nيرجى الرد:\n"
            "1️⃣ إرسال الطلب للتحدث مع موظف الاستقبال\n"
            "2️⃣ تعديل التاريخ/الفترة\n"
            "3️⃣ إلغاء"
            + _footer(lang)
        )
...
_DEPT_SYNONYMS: Dict[str, List[str]] = {
    ...
    "ophthal": [
        ...
        "قرنية", "رمد",
        "eye", "eye doctor", "eye specialist", "ophthalmology", "ophthalmologist",
        "ophthalmic", "optometry", "optometrist", "vision", "retina", "glaucoma", "cataract",
    ],
    "uro": [
...
_INFORMATIONAL_KEYWORDS_AR = [
    "تأمين", "التأمين", "بوبا", "تعاونية", "ميدغلف",
    "الموقع", "وين", "عنوان", "الاتجاهات",
    "الدوام", "اوقات", "أوقات", "ساعات", "متى تفتح", "متى تغلق",
    "الخدمات", "خدمات", "ماذا تقدمون", "شو عندكم",
    "الأطباء", "الاطباء", "مين عندكم", "من عندكم",
]
_INFORMATIONAL_KEYWORDS_EN = [
    "insurance", "bupa", "tawuniya", "medgulf",
    "location", "address", "where", "directions",
    "hours", "opening", "closing", "open", "close", "timings",
    "services", "service", "what do you offer",
    "doctors", "doctor list", "available doctors",
]
...
def _looks_like_informational_question(text: str) -> bool:
    ...
    direct_hits = [
        "ساعات العمل", "مواعيد العمل", "الدوام", "اوقات العمل", "أوقات العمل",
        "التأمين", "التأمينات", "تأمين",
        "الموقع", "العنوان", "وين", "أين",
        "رقم", "التواصل", "اتصال",
        "working hours", "hours", "timings",
        "insurance", "location", "address", "contact", "phone",
    ]
    ...
    for k in _INFORMATIONAL_KEYWORDS_AR + _INFORMATIONAL_KEYWORDS_EN:
        if _low(k) in t:
            return True
...
def _is_transaction_state(state: str) -> bool:
    return state in {
        STATE_BOOK_DEPT,
        STATE_BOOK_DOCTOR,
        STATE_BOOK_DATE,
        ...
        STATE_BOOK_PATIENT,
        STATE_BOOK_CONFIRM,
        STATE_RESCHEDULE_LOOKUP,
        STATE_RESCHEDULE_NEW_DATE,
        STATE_RESCHEDULE_NEW_SLOT,
...
def _safe_get_kb_answer(sess: Dict[str, Any], lang: str, message_text: str) -> Optional[str]:
    """
    Safe KB retrieval wrapper.
    Try tenant KB first.
    If no KB answer found, fall back to built-in FAQ text.
    """

    t = _low(message_text)

    tenant_id = str(
        (sess.get("meta") or {}).get("tenant_settings", {}).get("tenant_id")
        or sess.get("tenant_id")
        or ""
    ).strip()

    clinic_name = str(
        (sess.get("meta") or {}).get("tenant_settings", {}).get("clinic_name")
        or ""
    ).strip()

    # ---------------------------------------------------------
    # 1) Try tenant KB FIRST
    # ---------------------------------------------------------
    if build_knowledge_answer is not None and tenant_id:
        try:
            answer = asyncio.run(
                build_knowledge_answer(
                    db=None,  # placeholder, see note below
                    tenant_id=tenant_id,
        ...
        if answer:
            text_out = str(answer).strip()
            if text_out:
                return text_out + _footer(lang)

    # ---------------------------------------------------------
    # 2) Built-in safe fallback SECOND
    # ---------------------------------------------------------
    timings_hits = [
        "ساعات العمل", "مواعيد العمل", "الدوام", "اوقات العمل", "أوقات العمل",
        "working hours", "hours", "timings"
    ...
    insurance_hits = [
        "تأمين", "التأمين", "insurance", "insured", "accepted insurance"
    ]
    location_hits = [
        "الموقع", "العنوان", "وين", "أين", "location", "address", "map", "directions"
    ]
    contact_hits = [
        "رقم", "التواصل", "اتصال", "phone", "contact", "reception", "call"
    ]

    if any(_low(k) in t for k in timings_hits):
        return _timings_text(lang)

    if any(_low(k) in t for k in insurance_hits):
        return _insurance_text(lang)

    if any(_low(k) in t for k in location_hits):
        return _location_text(lang)

    if any(_low(k) in t for k in contact_hits):
        return _contact_text(lang)
...
def _doctor_info_reply(sess: Dict[str, Any], lang: str, dept_key: str) -> str:
    ...
    privacy = _maybe_privacy_once(sess, lang)
    if lang == "ar":
        return (
            f"مرحبًا بكم في *{CLINIC_NAME_AR}* 🏥\n\n"
...
def handle_turn(
    ...
    if not bool(sess.get("has_greeted")):
        ...
        if intent0 == "BOOK" and dept0 and dept0 in DOCTORS_BY_DEPT_KEY:
            ...
            if lang == "ar":
                out = (
                    f"مرحبًا بكم في *{CLINIC_NAME_AR}* 🏥\n\n"
                    f"تم اختيار تخصص *{sess['dept_label']}* ✅"
                    + privacy
                    + "\n\nيرجى اختيار الطبيب:\n\n"
                )
            else:
                out = (
                    f"Welcome to *{CLINIC_NAME_EN}* 🏥\n\n"
                    f"Selected specialty: *{sess['dept_label']}* ✅"
                    + privacy
        ...
        if _looks_like_informational_question(message_text):
            kb_answer = _safe_get_kb_answer(sess, lang, message_text)
            if kb_answer:
                sess["has_greeted"] = True
                sess["state"] = STATE_MENU
                sess["last_step"] = STATE_MENU
    ...
    if low == "99":
        ...
        out = (
            "تم تحويلكم للتحدث مع موظف الاستقبال ✅ الرجاء الانتظار... (للعودة للقائمة اكتب 0)"
            if lang == "ar" else
    ...
    if sess.get("state") == STATE_MENU:
        ...
        if _is_digit_choice(raw) and sess.get("intent") == "SPECIALTY_INQUIRY" and sess.get("dept_key"):
            ...
            if c == 1 and dept_key in DOCTORS_BY_DEPT_KEY:
                ...
                if lang == "ar":
                    out = f"تم اختيار تخصص *{sess['dept_label']}* ✅\n\n" + _doctor_prompt(lang, dept_key)
                else:
                    out = f"Selected specialty: *{sess['dept_label']}* ✅\n\n" + _doctor_prompt(lang, dept_key)
    ...
    if sess.get("state") == STATE_BOOK_DEPT:
        ...
        if _is_digit_choice(raw) and _to_int(raw) == 1 and raw.strip().startswith("0") and len(DEPTS) >= 10:
            raw = "10"
            low = "10"
    ...
    if sess.get("state") == STATE_BOOK_CONFIRM:
        ...
        if c == 2:
            ...
            sess["state"] = STATE_BOOK_DATE
            sess["last_step"] = STATE_BOOK_DATE
            out = ("تمام. اختر تاريخ جديد للموعد.\n\n" if lang == "ar" else "Okay. Please choose a new appointment date.\n\n")
            out += _date_prompt(lang)
            _set_bot(sess, out)
            return EngineResult(out, sess, [])
    ...
    if not _is_transaction_state(current_state) and _looks_like_informational_question(message_text):
        kb_answer = _safe_get_kb_answer(sess, lang, message_text)
        if kb_answer:
            out = kb_answer
            _set_bot(sess, out)
